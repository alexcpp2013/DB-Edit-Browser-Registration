...
namespace DBQ
{
    ...
    {        
        ...
        {
            //Зависит от имен контролов на форме пр иофрмировании запроса
            //Возможность использования автомтического имейла

            string requestUrl = MakeAddress(tbServerReferer.Text == "" ? tServer.Text : tbServerReferer.Text);
            if (tbServerReferer.Text == "")
                tbServerReferer.Text = tServer.Text;

            if(requestUrl == "")
                MessageBox.Show("Не задано имя сервера",
                                "Ошибка",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);

            string split = "/";
            if (!requestUrl.EndsWith(split) && !Referer.Text.StartsWith(split) && Referer.Text != "")
                requestUrl += split;
            requestUrl += Referer.Text;
            ...
            try
            {
                using (var client = new System.Net.WebClient())
                {
                    ...
                    var f = new List<Control>();
#if DEBUG
                    string val = "?";
#endif
                    foreach (Control control in this.gbValues.Controls)
                    {
                    ...
                    var answer = client.UploadValues(requestUrl, values);
                    rtbAnswer.Text = Encoding.UTF8.GetString(answer);
#if DEBUG
                ...
                doc2.write(rtbAnswer.Text);
                WebBrowserView.Navigate(rtbAnswer.Text);*/
                WebBrowserView.DocumentText = rtbAnswer.Text;

                //Обновить грид
                if (tbServerReferer.Text == tServer.Text)
                    button1_Click(sAdapter, e);
            }
            catch (Exception err)
            {
                MessageBox.Show("Произошла ошибка во время отправки запроса: \n\n" +
                                err.Message, "Ошибка",
                                MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void cbMail_CheckedChanged(object sender, EventArgs e)
        {
            if (cbMail.Checked == true)
                Email.Text = CompanyCode.Text + "@cc.com";
        }

        private void CompanyCode_TextChanged(object sender, EventArgs e)
        {
            if (cbMail.Checked == true)
                cbMail_CheckedChanged(sAdapter, e);
        }
    }
}
